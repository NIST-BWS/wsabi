...
-(id) init
{
	if ((self = [super init])) {
        ...
        self.pseudoResult = [[[WSBDResult alloc] init] autorelease];
...
-(AVCaptureVideoPreviewLayer*) previewLayer
{
    if (!self.captureManager) {
        ...
        NSError *error = nil;
        self.captureManager = [[[AVCamDemoCaptureManager alloc] init] autorelease];
        if ([self.captureManager setupSessionWithPreset:AVCaptureSessionPresetPhoto error:&error]) {
            
            previewLayer = [[[AVCaptureVideoPreviewLayer alloc] initWithSession:[captureManager session]] autorelease];
...
-(BOOL) beginCaptureSequence:(NSString *)sessionId captureType:(int)captureType withMaxSize:(float)maxSize withSenderTag:(int)senderTag
{
    ...
    [self beginConfigure:self.currentSessionId 
          withParameters:[NSDictionary dictionaryWithObjectsAndKeys:[WSBDModalityMap parameterNameForCaptureType:captureType],@"submodality",nil] 
...
-(void) beginRegisterClient:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeRegister 
...
{	
    ...
    [delegate sensorOperationCompleted:kOpTypeUnregister 
                              fromLink:self withResult:self.pseudoResult];
    ...
    if (self.sequenceInProgress) {
        ...
        [delegate sensorDisconnectSequenceCompletedFromLink:self withResult:self.pseudoResult shouldReleaseIfSuccessful:releaseIfSuccessful];
...
-(void) beginLock:(NSString*)sessionId withSenderTag:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeLock 
                              fromLink:self withResult:self.pseudoResult];
...
-(void) beginGetCommonInfo:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeGetCommonInfo 
                              fromLink:self withResult:self.pseudoResult];
...
-(void) beginGetDetailedInfo:(NSString*)sessionId withSenderTag:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeGetDetailedInfo
                              fromLink:self withResult:self.pseudoResult];
...
-(void) beginInitialize:(NSString*)sessionId withSenderTag:(int)senderTag
{
    ...
    if (self.sequenceInProgress) {
        self.sequenceInProgress = NO;
        [delegate sensorConnectSequenceCompletedFromLink:self withResult:self.pseudoResult];
    }
    operationInProgress = -1;
...
-(void) beginGetConfiguration:(NSString*)sessionId withSenderTag:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeGetConfiguration 
                              fromLink:self withResult:self.pseudoResult];
    operationInProgress = -1;
...
-(void) beginConfigure:(NSString*)sessionId withParameters:(NSDictionary*)params withSenderTag:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeConfigure 
                              fromLink:self withResult:self.pseudoResult];
    ...
    [[self.captureManager stillImageOutput] captureStillImageAsynchronouslyFromConnection:videoConnection
                                                                        completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
                                                                            if (imageDataSampleBuffer != NULL) {
                                                                                ...
                                                                                [delegate sensorOperationCompleted:kOpTypeCapture 
                                                                                                          fromLink:self withResult:self.pseudoResult];
...
-(void) beginGetCaptureInfo:(NSString*)captureId withSenderTag:(int)senderTag
{
   ...
   [delegate sensorOperationCompleted:kOpTypeGetContentType 
...
-(void) beginDownload:(NSString*)captureId withSenderTag:(int)senderTag
{
    ...
    WSBDResult *result = [[[WSBDResult alloc] init] autorelease];
...
-(void) beginCancel:(NSString*)sessionId withSenderTag:(int)senderTag
{
    ...
    [delegate sensorOperationCompleted:kOpTypeCancel 
                              fromLink:self withResult:self.pseudoResult];
    ...
    [delegate sensorOperationWasCancelledByClient:operationInProgress fromLink:self];
