...
-(id) init
{
	if ((self = [super init])) {
        ...
        cancelResponseData = [[NSMutableData data] retain];
...
-(void) downloadCompleted:(ASIHTTPRequest*)request
{
    ...
    if (!self.downloadSequenceResults) {
        self.downloadSequenceResults = [[[NSMutableArray alloc] init] autorelease];
    }
...
-(BOOL) parseResultData:(NSMutableData*)parseableData
{
	...
	NSString *checkReturnData = [[[NSString alloc] initWithData:parseableData encoding:NSUTF8StringEncoding] autorelease];
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	{
		self.currentParseResult = [[[WSBDResult alloc] init] autorelease];
	}
	else if ([elementName localizedCaseInsensitiveCompare:@"commonInfo"] == NSOrderedSame) {
		self.currentParseResult.infoCommon = [[[NSMutableDictionary alloc] init] autorelease];
		...
		self.currentParseResult.infoDetailed = [[[NSMutableDictionary alloc] init] autorelease];
		...
		self.currentParseResult.config = [[[NSMutableDictionary alloc] init] autorelease];
		...
		self.currentParseResult.captureIds = [[[NSMutableArray alloc] init] autorelease];
...
{
	[captureIds release];
	[currentSessionId release];
	[uri release];
	[currentParseResult release];
	[currentElementName release];
	[currentElementValue release];
	[currentContainerArray release];
	[currentContainerDictionary release];
	[currentDictionaryKey release];
	[currentDictionaryValue release];
	[acceptableContentTypes release];
	[downloadRetryCount release];
    
	[networkQueue reset];
	[networkQueue release];
    
    [responseData release];
    [cancelResponseData release];
	[super dealloc];
}
